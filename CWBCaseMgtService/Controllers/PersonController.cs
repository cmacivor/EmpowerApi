    ...
    public class PersonController : ApiController
    {
        ...
        public IEnumerable<dynamic> GetduplicatePersons(string id)
        {
            return (from p in context.Person
                    ...
                    where p.UniqueID.Contains(id)// && cp.Active == true //Change for SR1767028 to allow matching persons between systems (ie. person deleted from Juv) or previously deleted
                    select new
                    {
                        p.ID,
                        p.LastName,
        ...
        public async Task<object> Create(Person person)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var ssnresult = context.Person.Where(x => x.SSN == person.SSN && x.SSN != null && x.SSN != "" && x.UniqueID != null).FirstOrDefault();
        ...
        public async Task<object> Update(Person person)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var ssnresult = context.Person.Where(x => x.SSN == person.SSN && x.SSN != "" && x.SSN != null && x.UniqueID != null).FirstOrDefault();
        ...
        [System.Web.Http.HttpPost, Route("CreateNewClientProfile/{OriginalClientprofileID}")]
        public IHttpActionResult CreateNewClientProfile(string OriginalClientprofileID)
        {
            string result = "";
            ClientProfile clientProfile = new ClientProfile
            {
            ...
            if (OriginalClientprofileID != "" || OriginalClientprofileID != null)
            {
               
                // Create New ClientProfile
                try
                {
                    clientProfile.PersonID = Convert.ToInt32(OriginalClientprofileID);
                    clientProfile.CreatedDate = DateTime.Now;
                    clientProfile.UpdatedDate = DateTime.Now;
                    clientProfile.UpdatedBy = User.Identity.Name;
                    clientProfile.CreatedBy = User.Identity.Name;
                    clientProfileRepository.Create(clientProfile);
                    var cpVal = clientProfileRepository.Save();
                    result = "s";
