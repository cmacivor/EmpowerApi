...
SELECT
	...
	1 as [Active],
	CASE
		WHEN (SELECT TOP 1 k.ID FROM loginprofile k WHERE jp.CreatedBy = k.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 k.ID FROM loginprofile k WHERE jp.CreatedBy = k.AccountName) 
	END as [CreatedBy],
	CASE 
	...
	END as [CreatedDate], 
	CASE
		WHEN (SELECT TOP 1 m.ID FROM loginprofile m WHERE jp.UpdtBy = m.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 m.ID FROM loginprofile m WHERE jp.UpdtBy = m.AccountName) 
	END as [UpdatedBy],
	CASE 
...
SELECT 
	...
	1 as [CreatedBy], -- need to read from actual master table
	GetDate() as [CreatedDate], -- need to read from actual master table
	1 as [UpdatedBy], -- need to read from actual master table
...
SELECT
	...
	1 as [Active],
	CASE
		WHEN (SELECT TOP 1 k.ID FROM loginprofile k WHERE fp.CreatedBy = k.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 k.ID FROM loginprofile k WHERE fp.CreatedBy = k.AccountName) 
	END as [CreatedBy],
	CASE 
	...
	END as [CreatedDate], 
	CASE
		WHEN (SELECT TOP 1 m.ID FROM loginprofile m WHERE fp.UpdtBy = m.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 m.ID FROM loginprofile m WHERE fp.UpdtBy = m.AccountName) 
	END as [UpdatedBy],
	CASE 
...
SELECT 
DISTINCT 
	a.JuvenileId as [ClientProfilePersonID], 
	b.ID as [FamilyMemberID], 
	c.ID as [RelationshipID], 
	a.PrimaryContact as [PrimaryContactFlag], 
	CASE
		WHEN (SELECT TOP 1 k.ID FROM loginprofile k WHERE a.CreatedBy = k.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 k.ID FROM loginprofile k WHERE a.CreatedBy = k.AccountName) 
	END as [CreatedBy],
	CASE 
	...
	END as [CreatedDate], 
	CASE
		WHEN (SELECT TOP 1 m.ID FROM loginprofile m WHERE REPLACE(a.UpdtBy,'RICHVA\','') = m.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 m.ID FROM loginprofile m WHERE REPLACE(a.UpdtBy,'RICHVA\','') = m.AccountName) 
	END as [UpdatedBy],
	CASE 
...
SELECT 
	...
	1 as [MaritalStatusID], -- need to read from actual master table
	1 as [JobStatusID], -- need to read from actual master table
	...
	1 as [EducationLevelID], -- need to read from actual master table
...
-- Load data only for Internal type
...
SELECT
	...
	1 as [Active],
	CASE
		WHEN (SELECT TOP 1 k.ID FROM loginprofile k WHERE jadd.CreatedBy = k.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 k.ID FROM loginprofile k WHERE jadd.CreatedBy = k.AccountName) 
	END as [CreatedBy],
	CASE 
	...
	END as [CreatedDate], 
	CASE
		WHEN (SELECT TOP 1 m.ID FROM loginprofile m WHERE jadd.UpdatedBy = m.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 m.ID FROM loginprofile m WHERE jadd.UpdatedBy = m.AccountName) 
	END as [UpdatedBy],
	CASE 
...
-- Insert into PersonAddress from JJSCaseMgt.tblAddress 
-- Load data only for External type
INSERT INTO [dbo].[PersonAddress]
(
	[PersonID],
	[AddressTypeID],
...
SELECT
	p.ID as [PersonID],
	3 as [AddressTypeID],
	NULL as [GISCode], -- As Bing address no need for GIS
	NULL as [Latitude],
	NULL as [Longitude],
	RTRIM(
	 CASE WHEN jaddout.Building_Num <> '' THEN jaddout.Building_Num END
...
SELECT 
	...
	1 as [Active],
	CASE
		WHEN (SELECT TOP 1 k.ID FROM staff k WHERE tas.CreatedBy = k.NetLogin) IS NULL THEN 1 
		ELSE (SELECT TOP 1 k.ID FROM staff k WHERE tas.CreatedBy = k.NetLogin) 
	END as [CreatedBy],
	CASE 
	...
	END as [CreatedDate], 
	CASE
		WHEN (SELECT TOP 1 m.ID FROM staff m WHERE tas.UpdtBy = m.NetLogin) IS NULL THEN 1 
		ELSE (SELECT TOP 1 m.ID FROM staff m WHERE tas.UpdtBy = m.NetLogin) 
	END as [UpdatedBy],
	CASE 
...
--    Age as [AgeAtTime],
	...
	1 as [Active],
	CASE
		WHEN (SELECT TOP 1 k.ID FROM loginprofile k WHERE ref.CreatedBy = k.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 k.ID FROM loginprofile k WHERE ref.CreatedBy = k.AccountName) 
	END as [CreatedBy],
	CASE 
	...
	END as [CreatedDate], 
	CASE
		WHEN (SELECT TOP 1 m.ID FROM loginprofile m WHERE ref.UpdtBy = m.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 m.ID FROM loginprofile m WHERE ref.UpdtBy = m.AccountName) 
	END as [UpdatedBy],
	CASE 
...
SELECT
    ...
    refSer.DateofReferral as [DateofReferral],
	...
	1 as [Active],
	CASE
		WHEN (SELECT TOP 1 k.StaffID FROM [jjscasemgt].[dbo].[tblStaff] k WHERE refSer.CreatedBy = k.NetLogin) IS NULL THEN 1 
		ELSE (SELECT TOP 1 k.StaffID FROM [jjscasemgt].[dbo].[tblStaff] k WHERE refSer.CreatedBy = k.NetLogin) 
	END as [CreatedBy],
	CASE 
	...
	END as [CreatedDate], 
	CASE
		WHEN (SELECT TOP 1 m.StaffID FROM [jjscasemgt].[dbo].[tblStaff] m WHERE refSer.UpdtBy = m.NetLogin) IS NULL THEN 1 
		ELSE (SELECT TOP 1 m.StaffID FROM [jjscasemgt].[dbo].[tblStaff] m WHERE refSer.UpdtBy = m.NetLogin) 
	END as [UpdatedBy],
	CASE 
...
SELECT
	...
	NULL as [Duration],
	CASE
		WHEN (SELECT TOP 1 k.ID FROM loginprofile k WHERE REPLACE('RICHVA\', pn.AddBy, '') = k.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 k.ID FROM loginprofile k WHERE REPLACE('RICHVA\', pn.AddBy, '') = k.AccountName) 
	END as [CreatedBy],
	CASE 
	...
	END as [CreatedDate],
	CASE
		WHEN (SELECT TOP 1 m.ID FROM loginprofile m WHERE REPLACE('RICHVA\', pn.UpdtBy, '') = m.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 m.ID FROM loginprofile m WHERE REPLACE('RICHVA\', pn.UpdtBy, '') = m.AccountName) 
	END as [UpdatedBy],
	CASE 
...
SELECT
    ...
    su.Units as [Units],
	CASE
		WHEN (SELECT TOP 1 k.ID FROM loginprofile k WHERE su.CreatedBy = k.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 k.ID FROM loginprofile k WHERE su.CreatedBy = k.AccountName) 
	END as [CreatedBy],
	CASE 
	...
	END as [CreatedDate], 
	CASE
		WHEN (SELECT TOP 1 m.ID FROM loginprofile m WHERE su.UpdtBy = m.AccountName) IS NULL THEN 1 
		ELSE (SELECT TOP 1 m.ID FROM loginprofile m WHERE su.UpdtBy = m.AccountName) 
	END as [UpdatedBy],
	CASE 
