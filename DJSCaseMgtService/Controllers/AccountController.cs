...
//using DJSCaseMgtService.Results;
   ...
   [ Route("")]
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Remove(string name)
        {
            IdentityUser user = _userManager.FindByName(name);
            string Id = user.Id;
            var users = _userManager.Users.SingleOrDefault(u => u.Id == Id);          
        ...
        [System.Web.Http.HttpGet,Route("RegisterAll")]
        public async Task<IHttpActionResult> RegisterAll()
        { 

            IEnumerable<Staff> output = null;
            if (ModelState.IsValid)
            {
                output = context.Staff.Where(x => x.Active == true).ToList();
              
            }
            foreach (Staff list in output) {
                if (list.ID != 71 && list.NetLogin!= "browntd" && list.NetLogin != "rocketje" && list.NetLogin != "whitecm" && list.Password.Length<5) { 
                if (list.Agency == "DJS") {
                    UserModel user = new UserModel();

                    user.UserName = list.NetLogin;
                    user.Password = list.Password+ "empower";
                    user.ConfirmPassword = list.Password+"empower";
                    user.UserRole = list.Agency;
        ...
        public async Task<IList<string>> UserRoles()
        {
            ...
            try
            {
                ...
                roles = await _userManager.GetRolesAsync(userIdw);
                //var roleid = _userManager.FindById(User.Identity.GetUserId()).Roles.Select(r => r.RoleId);
            }
            catch(Exception ex)
            {
        ...
        //current user
        [System.Web.Http.HttpGet, Route("Username")]
